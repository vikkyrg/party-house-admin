...
const emptyRoom = { name: '', description: '', couple: 2, maximumMembers: 10, price: 0, features: '', amenities: '', slots: '10:00 AM - 01:00 PM', isActive: true };
...
export default function TheaterRoomsPage() {
  ...
  const submit = async (event) => {
    ...
    if (Number(form.maximumMembers) < Number(form.couple)) {
  ...
  const edit = (room) => setForm({ name: room.name, description: room.description || '', couple: room.couple ?? 2, maximumMembers: room.maximumMembers ?? 10, price: room.price ?? 0, features: (room.features || []).join(', '), amenities: (room.amenities || []).join(', '), slots: (room.slots || []).map((slot) => `${slot.startTime} - ${slot.endTime}`).join(', '), isActive: room.isActive });
  ...
  return <div className="flex h-full flex-col">
    ...
    <div className="grid gap-6 overflow-y-auto p-1 xl:grid-cols-[1fr_360px]">
      ...
      <form onSubmit={submit} className="space-y-3 rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="font-bold text-slate-900">{editingId ? 'Edit Room' : 'Add Room'}</h2>
        ...
        <div className="grid grid-cols-3 gap-3"><label className="text-sm font-medium text-slate-700">Couple *<input required type="number" min="1" value={form.couple} onChange={(event) => setForm({ ...form, couple: event.target.value })} className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" /></label><label className="text-sm font-medium text-slate-700">Maximum Members *<input required type="number" min="1" value={form.maximumMembers} onChange={(event) => setForm({ ...form, maximumMembers: event.target.value })} className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" /></label><label className="text-sm font-medium text-slate-700">Price / Hr *<input required type="number" min="0" value={form.price} onChange={(event) => setForm({ ...form, price: event.target.value })} className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" /></label></div>
        <label className="block text-sm font-medium text-slate-700">Room Image<input type="file" accept="image/*" onChange={(event) => setImage(event.target.files?.[0] || null)} className="mt-1 w-full text-sm" /></label>
